...
{
  $top_phone = get_theme_mod('newstore_top_phone');
  if (!empty($top_phone)) : ?>
  ...
  <?php endif; ?>
  <?php $top_email = get_theme_mod('newstore_top_email');
  if (!empty($top_email)) : ?>
  ...
  <?php endif; ?>
  <?php $top_address = get_theme_mod('newstore_top_address');
  if (!empty($top_address)) : ?>
...
function newstore_inline_css_more($custom_css)
{
  ...
  if (!empty($theme_primary_color) && $theme_primary_color != '#E91E63') {
    $custom_css .= "
  ...
  if (!empty($theme_link_color) && $theme_link_color != '#383838') {
    $custom_css .= "
			a{ color: $theme_link_color; }
		";
  }

  if (!empty($theme_link_hover_color) && $theme_link_hover_color != '#383838') {
    $custom_css .= "
			a:hover, a:focus { color: $theme_link_hover_color; }
		";
  }

  if (!empty($theme_post_title_color) && $theme_post_title_color != '#383838') {
    $custom_css .= "
			.entry-title.post-title a { color: $theme_post_title_color !important; }
		";
  }

  if (!empty($theme_post_title_hover_color) && $theme_post_title_hover_color != '#E91E63') {
    $custom_css .= "
			.entry-title.post-title a:hover { color: $theme_post_title_hover_color !important; }
		";
  }

  if (!empty($theme_post_meta_color) && $theme_post_meta_color != '#7a7a7a') {
    $custom_css .= "
			.post-meta-item i, .post-meta-item a { color: $theme_post_meta_color !important; }
		";
  }

  if (!empty($theme_post_meta_hover_color) && $theme_post_meta_hover_color != '#E91E63') {
    $custom_css .= "
...
if (!function_exists('newstore_get_google_fonts_url')) :
  function newstore_get_google_fonts_url($font_families = array('Open+Sans'))
  {
    $fonts_url = '';
    $font_families = apply_filters('newstore_google_fonts_array', $font_families);
    $query_args = array(
